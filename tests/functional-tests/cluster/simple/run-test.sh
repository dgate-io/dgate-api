...
# Wait for cluster to form and all nodes to join
wait_for_cluster() {
    log "Waiting for cluster to form..."
    
    # Wait for leader to be elected on node 1
    log "Waiting for leader election..."
    for i in {1..30}; do
        local status=$(curl -s --max-time $CURL_TIMEOUT "http://localhost:$NODE1_ADMIN_PORT/api/v1/cluster/status")
        local is_leader=$(echo "$status" | jq -r '.data.is_leader // false')
        if [[ "$is_leader" == "true" ]]; then
            log "Node 1 is leader"
            break
        fi
        sleep 0.5
    done
    
    # Wait for replication to be established to other nodes
    log "Waiting for cluster replication to be established..."
    for i in {1..30}; do
        local all_synced=true
        for port in $NODE2_ADMIN_PORT $NODE3_ADMIN_PORT; do
            local status=$(curl -s --max-time $CURL_TIMEOUT "http://localhost:$port/api/v1/cluster/status")
            local leader_id=$(echo "$status" | jq -r '.data.leader_id // null')
            if [[ "$leader_id" != "1" ]] && [[ "$leader_id" != "null" ]]; then
                all_synced=false
            fi
        done
        if $all_synced; then
            sleep 2  # Extra time for log replication
            break
        fi
        sleep 0.5
    done
    ...
    for port in $NODE1_ADMIN_PORT $NODE2_ADMIN_PORT $NODE3_ADMIN_PORT; do
        ...
        if [[ $(echo "$status" | jq -r '.data.enabled') != "true" ]]; then
            ...
            local leader=$(echo "$status" | jq -r '.data.leader_id // "unknown"')
...
for node_num in 1 2 3; do
    ...
    mode=$(echo "$status" | jq -r '.data.mode // "simple"')
