...
#
# NOTE: This test requires the full Raft implementation to be wired up.
# If using the simple replication stub, some tests may not pass.
...
NODE1_RAFT_PORT=9291
NODE2_RAFT_PORT=9292
NODE3_RAFT_PORT=9293
...
for node_num in 1 2 3; do
    ...
    metrics=$(curl -s "http://localhost:$port/api/v1/cluster/status")
